...
namespace Assets.Coop_New.Scripts.Missile
{
    public class Pod : MissilePositionLaunch
    {
        ...
        public override void Launch(Transform target, Vector3 velocity)
        {
            if (missileCount > 0 && reloadCooldown <= 0.0f && magazineReloadCooldown <= 0.0f)
            {
                ...
                MissileGenerate missile = CreateMissile(launchPoints[tubeCount].position, randomizedRotation);
        ...
        /// <returns></returns>
        private MissileGenerate CreateMissile(Vector3 position, Quaternion rotation)
        {
            MissileGenerate mis = Instantiate(missilePrefabToLaunch) as MissileGenerate;
            mis.ownShip = ownShip;
